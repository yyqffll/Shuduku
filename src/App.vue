<template>
  <div id="app">
    ...
    <transition-group name="sudoku-item-group" tag="p" class="sudoku-item-group">
    ...
    <div class="sudoku-item-group special-color">
...
export default {
  ...
  created () {
    ...
    this.sudokuCopy = this.deepCopy(this.sudoku)
    this.fillList = Array.from(Array(9), () => Array.from(Array(9)))
    this.fillListCopy = this.deepCopy(this.fillList)
  },
  methods: {
    // 获取数据类型
    getType (item) {
    ...
    move () {
      this.sudoku = this.deepCopy(this.sudokuCopy)
      this.fillList = this.deepCopy(this.fillListCopy)
    ...
    async randomList (sudoku) {
      const sudokuCopy = this.deepCopy(sudoku)
...
.input-show-enter-active {
  transition: all 3s ease;
}
.input-show-leave-active {
...
.special-color .sudoku-item:nth-of-type(2n + 1) {
  background: rgb(240, 240, 57);
}
.special-color .sudoku-item:nth-of-type(2n) {
